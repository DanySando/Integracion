...
def load_zones(zones_path: str, w: int, h: int):
    ...
    base_w = int(data.get("video_width") or w or 1)
    base_h = int(data.get("video_height") or h or 1)
    sx = float(w) / float(base_w) if base_w > 0 else 1.0
    sy = float(h) / float(base_h) if base_h > 0 else 1.0
...
def main():
    ...
    p.add_argument("--zones-file", default="zones.json",
                   help="Ruta a zones.json (para recalcular zonas)")
    p.add_argument("--use-zones", action="store_true",
                   help="Recalcula df.zone desde zones.json")
    ...
    # recalcular zonas con zones.json (para que el % por zona cambie si mueves zonas)
    if args.use_zones:
        polys = load_zones(args.zones_file, w, h)
        if polys is not None:
            df["zone"] = assign_zones(xs.astype(int), ys.astype(int), polys)

    if args.zone is not None and "zone" in df.columns:
        df = df[df["zone"] == args.zone]
        if df.empty:
            raise SystemExit("No quedó data tras filtrar por zona.")
        xs = df["cx"].to_numpy(dtype=np.float32)
        ys = df["cy"].to_numpy(dtype=np.float32)
    ...
    if first_frame is not None:
        overlay = cv2.addWeighted(
            first_frame, 1.0 - float(args.alpha), heat_color, float(args.alpha), 0.0)
    else:
    ...
    if args.legend:
        ...
        if has_valid_ids:
            ...
            for name, a, b in bins:
                ...
                pct = (c / n_people) * 100.0 if n_people > 0 else 0.0
